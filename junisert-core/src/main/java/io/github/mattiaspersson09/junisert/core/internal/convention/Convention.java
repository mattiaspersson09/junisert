...
public interface Convention {
    Predicate<Method> getterConvention(Field field);

    Predicate<Method> setterConvention(Field field);

    default String name() {
        return getClass().getSimpleName();
    }

    static Convention javaBeanCompliant() {
        return new JavaBeanConvention();
    }

    static Convention none() {
        return new NoConvention();
    }
}
